...
module.exports = {
  ...
  textTable:async(params)=>{
    
    let sql = `select * from textTable`;
    
    const data = await query(sql);
    return data;
  },
  findcardList:async(params)=>{
    
    let sql = `select * from cardList`;
  ...
  delItem:async(params)=>{
    ...
    let sql = `DELETE FROM cardList WHERE id = ${id}`;
    
    const data = await query(sql);
    return data;
  },
  //提交信息
  postTab: async (params) => {
    const {
      title, keyname, url, auth, icon, des,
    } = params;
    const sql = `
    INSERT INTO cardList(title, keyname, url, icon, auth, des) VALUES('${title}','${keyname}','${url}','${icon}','${auth}','${des}');
    `;
    await query(sql);
  },
  // based on categories
