...
router.post('/api/postTab', async (ctx) => {
  const { body } = ctx.request;
  await articles.postTab(body).then(() => {
    ctx.body = {
      message: '保存成功',
      code: 1,
    };
 ...
 * 获取text
 */
router.get('/api/textTable', async (ctx) => {
  const params = ctx.request.query;
  const res = await articles.textTable(params);
  ctx.body = res;
});
/**
 * 获取text
 */
router.get('/api/findcardList', async (ctx) => {
  const params = ctx.request;
  const res = await articles.findcardList(params);
  ctx.body = res;
});/**
* 获取text
*/
router.post('/api/del', async (ctx) => {
  const { body } = ctx.request;
 await articles.delItem(body).then(() => {
  ctx.body = {
    message: '删除成功',
    code: 1,
  };
